public class Practice_5_13_Perfect {
    //静态方法
    public static void perPayment_1(int perInstalled) {
        /*if (perInstalled == 3){
    ...
    public static void perPayment_2(int num) {
        int perInstalled = num == 1 ? 3 : (num == 2 ? 6 : 12);
        double totalInterest = num == 1 ? (102.50 / 100) : (num == 2 ? (104.50 / 100) : (108.80 / 100));
        double perPayment = (52.10 * totalInterest) / perInstalled;
        switch (num) {
            case 1:
                /*double res_1 = (52.10 * 102.50 / 100) / 3;
                System.out.println("分3期还款，每期的还款金额为：" + res_1 + "元");*/
                System.out.println("分3期还款，每期的还款金额为：" + perPayment + "元");
                break;
            case 2:
                /*double res_2 = (52.10 * 104.50 / 100) / 6;
                System.out.println("分6期还款，每期的还款金额为：" + res_2 + "元");*/
                System.out.println("分6期还款，每期的还款金额为：" + perPayment + "元");
                break;
            default:
                /*double res_3 = (52.10 * 108.80 / 100) / 12;
                System.out.println("分12期还款，每期的还款金额为：" + res_3 + "元");*/
                System.out.println("分12期还款，每期的还款金额为：" + perPayment + "元");
        }
    }
    ...
    public static void main(String[] args) {
        ...
        float score1 = 90.5F;//小慕的成绩
        float score2 = 99.8F;//大毛的成绩
        System.out.println(score1 == score2 ? "小慕和大毛的成绩相同" : "小慕和大毛的成绩不相同");
        System.out.println("");
        ...
        if (x >= 80 || y++ >= 90) {
            System.out.println( "y的值是：" + y) ;//95: x >= 80 为 true，发生短路求值，y++ 不执行，直接打印语句
        }
        if (x >= 90 || y++ >= 90) {
            System.out.println( "y的值是：" + y) ;//96: x >= 90 为 false，再判断 y >= 90 为 true，接着执行 y++ ==> y = 96,最后打印语句
        }
        if (x >= 90 && y++ >= 90) {
            System.out.println( "y的值是：" + y) ;//不执行：x >= 90 为 false，发生短路求值，y++ 和打印语句都不执行，且 y 仍为 96
        }
        System.out.println("");
