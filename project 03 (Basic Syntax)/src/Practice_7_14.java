public class Practice_7_14 {
    ...
    public static void bubbleSort() {
        float[] avgScore =  new float[]{96.15F, 95.7F, 94.35F, 95.9F, 95.1F};
        for (int i =0 ;i < avgScore.length - 1;i++) {
            for (int j = 0;j < avgScore.length - 1- i; j++) {
                if (avgScore[j] > avgScore[j+1]) {
                    float temp = avgScore[j];
                    avgScore[j] = avgScore[j+1];
                    avgScore[j+1] = temp;
                }
            }
            System.out.print("第" + i + "轮冒泡排序：");
            for (float c : avgScore) {
                System.out.print(c + "\t");
            }
            System.out.println();
        }
    ...
    public static void main(String[] args) {
        ...
        //应该将i <= arr.length 改为 i < arr.length，因为arr[11]可能会有数组越界风险
        System.out.println("");
